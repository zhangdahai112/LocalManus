...
- **浏览器生命周期管理改进**：增强的logging和资源清理优化，特别是在截图捕获、搜索结果解析和内容提取功能中的改进
...
**更新** 完全重构为Playwright/CDP直接浏览器自动化方式，新增浏览器控制方法，提供更简洁的架构和更强的功能。浏览器生命周期管理得到显著改进，包括增强的logging和资源清理优化。
...
**图表来源**
- [main.py:1-519](file://localmanus-backend/main.py#L1-L519)
...
**图表来源**
- [skill_manager.py:78-216](file://localmanus-backend/core/skill_manager.py#L78-L216)
- [web_tools.py:214-571](file://localmanus-backend/skills/web-search/web_tools.py#L214-L571)
- [firecracker_sandbox.py:121-312](file://localmanus-backend/core/firecracker_sandbox.py#L121-L312)
...
**图表来源**
- [react_agent.py:20-250](file://localmanus-backend/agents/react_agent.py#L20-L250)
- [skill_manager.py:150-216](file://localmanus-backend/core/skill_manager.py#L150-L216)
- [web_tools.py:240-571](file://localmanus-backend/skills/web-search/web_tools.py#L240-L571)
...
**图表来源**
- [main.py:33-519](file://localmanus-backend/main.py#L33-L519)
- [orchestrator.py:11-158](file://localmanus-backend/core/orchestrator.py#L11-L158)
- [firecracker_sandbox.py:121-312](file://localmanus-backend/core/firecracker_sandbox.py#L121-L312)
...
**图表来源**
- [web_tools.py:240-284](file://localmanus-backend/skills/web-search/web_tools.py#L240-L284)

#### 搜索功能实现

**更新** 搜索功能现在包含增强的浏览器生命周期管理和资源清理优化：
...
FormatOutput --> ClosePage["关闭页面资源"]
ClosePage --> ReturnResults["返回搜索结果"]
ReturnError --> End([结束])
ReturnResults --> End
```

**图表来源**
- [web_tools.py:286-323](file://localmanus-backend/skills/web-search/web_tools.py#L286-L323)

#### 内容抓取功能实现

**更新** 内容抓取功能现在包含改进的资源清理和增强的logging：
...
LimitLength --> ClosePage["关闭页面资源"]
ClosePage --> ReturnText["返回处理后的文本"]
ReturnURLError --> End([结束])
ReturnText --> End
```

**图表来源**
- [web_tools.py:325-344](file://localmanus-backend/skills/web-search/web_tools.py#L325-L344)

#### 截图功能实现

**更新** 截图功能现在包含增强的logging和资源清理优化：
...
TakeScreenshot --> ClosePage["关闭页面资源"]
ClosePage --> EncodeBase64["编码为Base64"]
...
**图表来源**
- [web_tools.py:346-383](file://localmanus-backend/skills/web-search/web_tools.py#L346-L383)
...
ClickElement --> LogAction["记录操作日志"]
LogAction --> ReturnResult["返回操作结果"]
TypeText --> LogAction
ScrollPage --> LogAction
ReturnResult --> End([结束])
```

**图表来源**
- [web_tools.py:385-454](file://localmanus-backend/skills/web-search/web_tools.py#L385-L454)
...
**图表来源**
- [web_tools.py:24-53](file://localmanus-backend/skills/web-search/web_tools.py#L24-L53)
- [web_tools.py:55-90](file://localmanus-backend/skills/web-search/web_tools.py#L55-L90)
- [firecracker_sandbox.py:121-312](file://localmanus-backend/core/firecracker_sandbox.py#L121-L312)
...
**图表来源**
- [web_tools.py:532-571](file://localmanus-backend/skills/web-search/web_tools.py#L532-L571)
...
**更新** WebSearchSkill 实现了完善的错误处理机制，包括沙箱可用性检查、CDP连接错误处理和Playwright浏览器异常处理，以及增强的logging：

| 错误类型 | 触发条件 | 处理方式 | 返回信息 | 日志记录 |
|---------|---------|---------|---------|---------|
| 沙箱不可用 | 沙箱未运行或无法连接 | 捕获SandboxBrowserError并返回详细错误 | "❌ Sandbox Error: {错误详情}" | 记录错误日志 |
| CDP连接失败 | 浏览器CDP连接建立失败 | 捕获异常并返回错误信息 | "❌ Sandbox Error: Failed to connect to sandbox browser at CDP URL" | 记录连接失败日志 |
| Playwright安装缺失 | Playwright包未安装 | 捕获ImportError并返回安装指引 | "playwright is not installed. Run: pip install playwright && playwright install chromium" | 记录安装错误日志 |
| 搜索异常 | 搜索超时或解析错误 | 捕获异常并返回错误信息 | "Search error: {错误详情}" | 记录搜索错误日志 |
| 网页抓取异常 | 页面加载失败或内容提取错误 | 捕获异常并返回详细错误 | "Scrape error: {错误详情}" | 记录抓取错误日志 |
| 截图异常 | 页面截图失败或编码错误 | 捕获异常并返回错误信息 | "Screenshot error: {错误详情}" | 记录截图错误日志 |
| 浏览器控制异常 | 元素点击/输入/滚动失败 | 捕获异常并返回错误信息 | "Click/Type/Scroll error: {错误详情}" | 记录控制操作错误日志 |
| 网络超时 | 请求超过30秒未响应 | 捕获超时异常 | 超时错误信息 | 记录超时警告 |
| 未知错误 | 未预期的异常情况 | 捕获并返回通用错误信息 | "❌ Unexpected error: {错误详情}" | 记录未知错误 |

**更新** 新增了详细的logging记录，包括连接成功、页面关闭、操作执行等关键节点的日志记录，提供更好的调试和监控能力。

**章节来源**
- [web_tools.py:19-21](file://localmanus-backend/skills/web-search/web_tools.py#L19-L21)
- [web_tools.py:34-50](file://localmanus-backend/skills/web-search/web_tools.py#L34-L50)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L322)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L343)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L382)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L405)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L429)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L453)
...
**图表来源**
- [skill_manager.py:89-149](file://localmanus-backend/core/skill_manager.py#L89-L149)
...
**章节来源**
- [skill_manager.py:137-140](file://localmanus-backend/core/skill_manager.py#L137-L140)
...
**章节来源**
- [react_agent.py:114-227](file://localmanus-backend/agents/react_agent.py#L114-L227)
...
**图表来源**
- [page.tsx:89-131](file://localmanus-ui/app/page.tsx#L89-L131)
...
**章节来源**
- [page.tsx:1-297](file://localmanus-ui/app/page.tsx#L1-L297)
...
**章节来源**
- [.env.example:1-23](file://localmanus-backend/.env.example#L1-L23)

## 性能考虑

### Playwright/CDP优化

**更新** WebSearchSkill 在浏览器连接方面采用了多项优化措施：
...
5. **资源清理**: 确保浏览器操作完成后正确关闭页面和清理资源，包含增强的logging
6. **错误快速失败**: 通过沙箱可用性检查和CDP连接检查快速发现和报告问题
7. **浏览器生命周期管理**: 改进的页面创建和销毁流程，确保资源及时释放
...
StatusCheck -.-> Logging[增强Logging]
Logging -.-> ResourceCleanup[资源清理优化]
ResourceCleanup -.-> PageClose[页面关闭优化]
PageClose -.-> Success[操作成功]
End
```
...
| 问题类型 | 症状 | 可能原因 | 解决方案 | 日志参考 |
|---------|------|---------|---------|---------|
| 沙箱不可用 | "❌ Sandbox Error: No sandbox available for user {id}" | 沙箱服务未运行或无法连接 | 检查沙箱服务状态，确保Docker容器正在运行 | 沙箱连接日志 |
| CDP连接失败 | "❌ Sandbox Error: Failed to connect to sandbox browser at CDP URL" | CDP WebSocket连接建立失败 | 检查SANDBOX_LOCAL_URL配置，验证CDP端点可达性 | CDP连接失败日志 |
| Playwright安装缺失 | "playwright is not installed. Run: pip install playwright && playwright install chromium" | Playwright包未安装 | 运行pip install playwright && playwright install chromium命令 | 安装错误日志 |
| 智能CDP URL解析失败 | "Cannot get browser CDP URL for user {id}" | 无法从容器获取CDP URL或构造失败 | 检查沙箱容器是否正确暴露CDP端点 | CDP URL解析日志 |
| 搜索无结果 | "No results found." | 沙箱浏览器连接失败 | 检查沙箱服务状态，重启Docker容器 | 搜索错误日志 |
| 抓取失败 | "Scrape error: ..." | 页面加载超时或内容提取错误 | 检查目标网站可达性，增加等待时间 | 抓取错误日志 |
| 截图失败 | "Screenshot error: ..." | 页面截图失败或编码错误 | 检查浏览器权限，确认页面渲染完成 | 截图错误日志 |
| 浏览器控制失败 | "Click/Type/Scroll error: ..." | 元素选择器无效或浏览器状态异常 | 检查CSS选择器或XPath，确认元素存在 | 控制操作错误日志 |
| 超时错误 | 连接超时 | 网络延迟或沙箱服务繁忙 | 增加超时时间，检查Docker资源限制 | 超时警告日志 |
| 浏览器连接失败 | CDP连接建立失败 | Docker容器未启动或端口冲突 | 启动沙箱容器，检查端口占用 | 连接失败日志 |
| 工具调用失败 | "Error: Tool 'search' not found." | 使用过时的方法名 | 更新为 'search_web' 或 'scrape_web' | 未知错误日志 |
| 资源泄漏 | 内存使用持续增长 | 页面未正确关闭或资源未清理 | 检查页面关闭流程，确认finally块执行 | 资源清理日志 |

**更新** 新增了Playwright/CDP相关故障排除指南，指导用户解决浏览器状态检查和浏览器控制问题。新增了详细的日志参考，帮助用户定位问题。
...
8. **资源使用监控**: 监控浏览器池大小和内存使用情况
9. **页面生命周期跟踪**: 确认页面创建和关闭的配对调用
10. **错误日志分析**: 查看详细的异常堆栈信息和上下文

**章节来源**
- [web_tools.py:98-108](file://localmanus-backend/skills/web-search/web_tools.py#L98-L108)
- [web_tools.py:46-50](file://localmanus-backend/skills/web-search/web_tools.py#L46-L50)
- [web_tools.py:85-90](file://localmanus-backend/skills/web-search/web_tools.py#L85-L90)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L322)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L343)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L382)
- [web_tools.py](file://localmanus-backend/skills/web-search/web_tools.py#L405)
...
**更新** 本次更新实现了类名和方法名的标准化，完全重构为Playwright/CDP直接浏览器自动化方式，新增了浏览器控制方法和状态检查功能，提供了更简洁的架构和更强的功能，同时通过兼容性包装类确保了向后兼容性。浏览器生命周期管理得到显著改进，包括增强的logging和资源清理优化，特别是在截图捕获、搜索结果解析和内容提取功能中的改进。
...
8. **增强错误处理**: 完善的异常捕获和恢复机制，包含详细的logging
9. **性能优化**: 浏览器池管理和资源清理，改进的页面生命周期管理
10. **向后兼容**: 兼容性包装类确保现有代码正常运行
11. **前端友好**: 实时消息流支持优秀的用户体验
12. **资源管理优化**: 改进的页面创建/销毁流程，确保资源及时释放
13. **调试支持增强**: 详细的日志记录，便于问题诊断和性能监控
